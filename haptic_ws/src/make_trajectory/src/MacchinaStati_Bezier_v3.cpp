...
coefficients calc_coefficients(coefficients *coeff,std::vector<double> pos_init,std::vector<double>joint_pos_init,std::vector<double> pos_fin) {
   // std::cout <<"IN CALC_COEFFICIENTS\n" ;
    ...
    std::vector<double> direzione_ctrl_1 ; 
    direzione_ctrl_1.resize(3) ; 
    direzione_ctrl_1[0] = 2*(pos_init[4]*pos_init[6]+pos_init[5]*pos_init[3]) ; 
    direzione_ctrl_1[1] = 2*(pos_init[5]*pos_init[6]-pos_init[4]*pos_init[3]) ; 
    direzione_ctrl_1[2] = pow(pos_init[3],2)-pow(pos_init[4],2)-pow(pos_init[5],2)+pow(pos_init[6],2) ; 
    for(int i=0;i<3;i++) pos_ctrl_1[i] = proiezione * direzione_ctrl_1[i] +pos_init[i] ; 
